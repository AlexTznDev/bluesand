...
window.Webflow.push(() => {
  ...
  ───────────────────────────────────────────── */
      function buildPacingTimeline(scene) {
        ...
        if (counter) animateCounter(counter, 92, 1.2, 0.4);
        if (lineExp)
          tl.to(lineExp, { strokeDashoffset: 0, duration: 1.6, ease: 'power1.inOut' }, 0.55);
        if (lineAct)
          tl.to(lineAct, { strokeDashoffset: 0, duration: 1.6, ease: 'power1.inOut' }, 0.75);
        ...
        if (dots.length) {
          tl.to(
            dots,
            { opacity: 1, scale: 1, duration: 0.4, stagger: 0.08, ease: 'back.out(2.5)' },
            1.95
          );
        }
        return tl;
